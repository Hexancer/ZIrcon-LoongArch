...
void arch_spin_lock(spin_lock_t* lock) TA_NO_THREAD_SAFETY_ANALYSIS {
    unsigned long val = arch_curr_cpu_num() + 2;
    uint64_t temp;
...
int arch_spin_trylock(spin_lock_t* lock) TA_NO_THREAD_SAFETY_ANALYSIS {
    ...
    __asm__ volatile(
        "ll.d   %[out], %[lock], 0;"  // load link from %lock
        "bnez   %[out], 1f;"          // %lock != 0 <==> some core holding it
        "move   %[out], %[val];"      // prepare val to write
        "sc.d   %[out], %[lock], 0;"  // store cond to %lock
        "1:"
        : [out] "=&r"(out)
        : [lock] "r"(&lock->value), [val] "r"(val)
        : "cc", "memory");
